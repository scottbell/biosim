	...
	implements com.traclabs.biosim.idl.simulation.water.PotableWaterStore
{
	...
	private String[] ids = {"IDL:com/traclabs/biosim/idl/simulation/water/PotableWaterStore:1.0","IDL:com/traclabs/biosim/idl/simulation/framework/PassiveModule:1.0","IDL:com/traclabs/biosim/idl/simulation/water/WaterStore:1.0","IDL:com/traclabs/biosim/idl/simulation/framework/Store:1.0"};
	public String[] _ids()
	{
		return ids;
	}
		...
		while(true)
		{
			if(! this._is_local())
			{
				...
				try
				{
					...
					java.lang.String tmpResult124 = commandName;
	...
	public float getCurrentTemperature()
	{
		while(true)
		{
			if(! this._is_local())
			{
				...
				try
				{
					_os = _request( "getCurrentTemperature", true);
					_is = _invoke(_os);
					float _result = _is.read_float();
					return _result;
				}
	...
	public float addWaterWithTemperature(float mass, float temperature)
	{
		while(true)
		{
			if(! this._is_local())
			{
				...
				try
				{
					_os = _request( "addWaterWithTemperature", true);
	...
	public void setInitialWaterTemperature(float temperature)
	{
		while(true)
		{
			if(! this._is_local())
			{
				...
				try
				{
					_os = _request( "setInitialWaterTemperature", true);
					_os.write_float(temperature);
					_is = _invoke(_os);
					return;
				}
