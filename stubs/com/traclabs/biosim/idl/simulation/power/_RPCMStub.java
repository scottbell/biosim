	...
	public com.traclabs.biosim.idl.simulation.power.RPCMSwitchState getSwitchState()
	{
		while(true)
		{
			if(! this._is_local())
			{
				...
				try
				{
					_os = _request( "getSwitchState", true);
					_is = _invoke(_os);
		...
		else
		{
			...
			if( _so == null )
				continue;
			RPCMOperations _localServant = (RPCMOperations)_so.servant;
			com.traclabs.biosim.idl.simulation.power.RPCMSwitchState _result;
			try
			{
		...
		while(true)
		{
			if(! this._is_local())
			{
				...
				try
				{
					...
					java.lang.String tmpResult82 = commandName;
	...
	public com.traclabs.biosim.idl.simulation.power.RPCMArmedStatus getArmedStatus()
	{
		...
		else
		{
			...
			if( _so == null )
				continue;
			RPCMOperations _localServant = (RPCMOperations)_so.servant;
			com.traclabs.biosim.idl.simulation.power.RPCMArmedStatus _result;
			try
			{
	...
	public void setSwitchState(com.traclabs.biosim.idl.simulation.power.RPCMSwitchState state)
	{
		while(true)
		{
			if(! this._is_local())
			{
				...
				try
				{
					_os = _request( "setSwitchState", true);
