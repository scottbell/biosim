...
public class OnlineRedundancyController implements BiosimController {
	...
	public OnlineRedundancyController(boolean log) {
		logToFile = log;
	...
	public static void main(String[] args) {
		...
		//int max = 4;
		//for (int i = 0; i < max; i++) {
			OnlineRedundancyController myController = new OnlineRedundancyController(logToFile);
	...
	public void stepSim() {
		...
		} else if (myO2PressureSensor.getValue() < 15 && line1active && line2active) {
	...
	private void line2_fail(){
		 if(OGS2active && O2Storeactive2 && CO2Storeactive2 && Injector2active && VCCR2active && H2Storeactive2 && WRS2active && DirtyWaterStoreactive2 && GreyWaterStoreactive2 && PotableWaterStoreactive2){
			 line2active=true;
	...
	public void assign_values() {
		...
		if (!line1active && line2active) {
			...
			myBioHolder.theDehumidifiers.get(1).getAirConsumerDefinition().setDesiredFlowRate(2*DehumidifierAirConsumer, 0);
			myBioHolder.theDehumidifiers.get(1).getDirtyWaterProducerDefinition().setDesiredFlowRate(2*DehumidifierDirtyWaterProducer, 0);
		...
		if (!line2active && line1active) {
			...
			myBioHolder.theDehumidifiers.get(0).getAirConsumerDefinition().setDesiredFlowRate(2*DehumidifierAirConsumer, 0);
			myBioHolder.theDehumidifiers.get(0).getDirtyWaterProducerDefinition().setDesiredFlowRate(2*DehumidifierDirtyWaterProducer, 0);
			myBioHolder.theDehumidifiers.get(1).getAirConsumerDefinition().setDesiredFlowRate(0, 0);
			myBioHolder.theDehumidifiers.get(1).getDirtyWaterProducerDefinition().setDesiredFlowRate(0, 0);
			myBioHolder.theWaterRSModules.get(1).getPowerConsumerDefinition().setDesiredFlowRate(0, 0);
			myBioHolder.theOGSModules.get(1).getPowerConsumerDefinition().setDesiredFlowRate(0, 0);
			myBioHolder.theVCCRModules.get(1).getPowerConsumerDefinition().setDesiredFlowRate(0, 0);
			myBioHolder.theInjectors.get(1).getO2ConsumerDefinition().setDesiredFlowRate(0, 0);
			myBioHolder.theInjectors.get(1).getO2ProducerDefinition().setDesiredFlowRate(0, 0);
			myBioHolder.theInjectors.get(1).getCO2ConsumerDefinition().setDesiredFlowRate(0, 0);
			myBioHolder.theInjectors.get(1).getCO2ProducerDefinition().setDesiredFlowRate(0, 0);
		}
