...
public class IncineratorImpl extends SimBioModuleImpl implements IncineratorOperations, PowerConsumerOperations, DryWasteConsumerOperations, O2ConsumerOperations, CO2ProducerOperations{
	...
	//Flag to determine if the Incinerator has enough dryWaste to function nominally
	...
	//The dryWaste consumed (in kilograms) by the Incinerator at the current tick
	...
	*/
	...
	/**
	* Returns the dryWaste consumed (in kilograms) by the Incinerator during the current tick
	* @return the dryWaste consumed (in kilograms) by the Incinerator during the current tick
	*/
	...
	/**
	* Returns the power consumed (in watts) by the Incinerator during the current tick
	* @return the power consumed (in watts) by the Incinerator during the current tick
	*/
	...
	/**
	* Returns the O2 consumed (in moles) by the Incinerator during the current tick
	* @return the O2 consumed (in moles) by the Incinerator during the current tick
	*/
	...
	/**
	* Returns the CO2 produced (in moles) by the Incinerator during the current tick
	* @return the CO2 produced (in moles) by the Incinerator during the current tick
	*/
	...
	* @return <code>true</code> if the Incinerator has enough power, <code>false</code> if not.
	*/
	...
	* @return <code>true</code> if the Incinerator has enough dryWaste, <code>false</code> if not.
	*/
	...
	* @return <code>true</code> if the Incinerator has enough O2, <code>false</code> if not.
	*/
	...
	/**
	* Attempts to collect enough power from the Power PS to run the Incinerator for one tick.
	*/
	...
	/**
	* Attempts to collect enough dryWaste from the DryWaste Store to run the Incinerator optimally for one tick.
	*/
	private void gatherDryWaste(){
		currentDryWasteConsumed = getResourceFromStore(myDryWasteStores, dryWasteMaxFlowRates, dryWasteDesiredFlowRates, dryWasteActualFlowRates, dryWasteNeeded);
	...
	*/
	...
	/**
	* When ticked, the Incinerator does the following: 
	* 1) attempts to collect references to various server (if not already done).
	* 2) consumes power and dryWaste.
	...
	*/
	...
	public void log(){
		...
		if (!logInitialized){
			...
			LogNode dryWasteNeededHead = myLog.addChild("dryWaste_needed");
			myLogIndex.dryWasteNeededIndex = dryWasteNeededHead.addChild(""+dryWasteNeeded);
			LogNode currentDryWasteConsumedHead = myLog.addChild("current_dryWaste_consumed");
