...
import com.traclabs.biosim.idl.framework.CO2AirConsumerOperations;
import com.traclabs.biosim.idl.framework.CO2AirProducerOperations;
...
import com.traclabs.biosim.idl.framework.FoodConsumerOperations;
...
import com.traclabs.biosim.idl.framework.GreyWaterProducerOperations;
import com.traclabs.biosim.idl.framework.MalfunctionIntensity;
import com.traclabs.biosim.idl.framework.MalfunctionLength;
import com.traclabs.biosim.idl.framework.O2AirConsumerOperations;
import com.traclabs.biosim.idl.framework.O2AirProducerOperations;
...
public class AccumulatorImpl extends SimBioModuleImpl implements
        ...
        PowerProducerOperations, PotableWaterProducerOperations,
        GreyWaterProducerOperations, DirtyWaterProducerOperations,
        O2ProducerOperations, CO2ProducerOperations, AirProducerOperations,
        BiomassProducerOperations, FoodProducerOperations,
        O2AirConsumerOperations, CO2AirConsumerOperations,
        O2AirProducerOperations, CO2AirProducerOperations,
        DryWasteProducerOperations, DryWasteConsumerOperations {
    ...
    private void getAndPushResources() {
        ...
        float powerPushed = pushResourceToStore(myPowerOutputs,
                powerOutMaxFlowRates, powerOutDesiredFlowRates,
                powerOutActualFlowRates, powerGathered);

        float greyWaterGathered = getMostResourceFromStore(myGreyWaterInputs,
                greyWaterInMaxFlowRates, greyWaterInDesiredFlowRates,
                greyWaterInActualFlowRates);
        float greyWaterPushed = pushResourceToStore(myGreyWaterOutputs,
                greyWaterOutMaxFlowRates, greyWaterOutDesiredFlowRates,
                greyWaterOutActualFlowRates, greyWaterGathered);

        float potableWaterGathered = getMostResourceFromStore(
                myPotableWaterInputs, potableWaterInMaxFlowRates,
                potableWaterInDesiredFlowRates, potableWaterInActualFlowRates);
        float potableWaterPushed = pushResourceToStore(myPotableWaterOutputs,
                potableWaterOutMaxFlowRates, potableWaterOutDesiredFlowRates,
                potableWaterOutActualFlowRates, potableWaterGathered);

        float dirtyWaterGathered = getMostResourceFromStore(myDirtyWaterInputs,
                dirtyWaterInMaxFlowRates, dirtyWaterInDesiredFlowRates,
                dirtyWaterInActualFlowRates);
        float dirtyWaterPushed = pushResourceToStore(myDirtyWaterOutputs,
                dirtyWaterOutMaxFlowRates, dirtyWaterOutDesiredFlowRates,
                dirtyWaterOutActualFlowRates, dirtyWaterGathered);

        float biomassGathered = getMostResourceFromStore(myBiomassInputs,
                biomassInMaxFlowRates, biomassInDesiredFlowRates,
                biomassInActualFlowRates);
        float biomassPushed = pushResourceToStore(myBiomassOutputs,
                biomassOutMaxFlowRates, biomassOutDesiredFlowRates,
                biomassOutActualFlowRates, biomassGathered);

        float foodGathered = getMostResourceFromStore(myFoodInputs,
                foodInMaxFlowRates, foodInDesiredFlowRates,
                foodInActualFlowRates);
        float foodPushed = pushResourceToStore(myFoodOutputs,
                foodOutMaxFlowRates, foodOutDesiredFlowRates,
                foodOutActualFlowRates, foodGathered);

        float dryWasteGathered = getMostResourceFromStore(myDryWasteInputs,
                dryWasteInMaxFlowRates, dryWasteInDesiredFlowRates,
                dryWasteInActualFlowRates);
        float dryWastePushed = pushResourceToStore(myDryWasteOutputs,
                dryWasteOutMaxFlowRates, dryWasteOutDesiredFlowRates,
                dryWasteOutActualFlowRates, dryWasteGathered);

        float O2Gathered = getMostResourceFromStore(myO2Inputs,
                O2InMaxFlowRates, O2InDesiredFlowRates, O2InActualFlowRates);
        float O2Pushed = pushResourceToStore(myO2Outputs, O2OutMaxFlowRates,
                O2OutDesiredFlowRates, O2OutActualFlowRates, O2Gathered);

        float CO2Gathered = getMostResourceFromStore(myCO2Inputs,
                CO2InMaxFlowRates, CO2InDesiredFlowRates, CO2InActualFlowRates);
        float CO2Pushed = pushResourceToStore(myCO2Outputs, CO2OutMaxFlowRates,
                CO2OutDesiredFlowRates, CO2OutActualFlowRates, CO2Gathered);

        float nitrogenGathered = getMostResourceFromStore(myNitrogenInputs,
                nitrogenInMaxFlowRates, nitrogenInDesiredFlowRates,
                nitrogenInActualFlowRates);
        float nitrogenPushed = pushResourceToStore(myNitrogenOutputs,
                nitrogenOutMaxFlowRates, nitrogenOutDesiredFlowRates,
                nitrogenOutActualFlowRates, nitrogenGathered);
        ...
        Breath distributedAir = new Breath(gatheredAir.O2, gatheredAir.CO2,
                gatheredAir.water, gatheredAir.other, gatheredAir.nitrogen);
        for (int i = 0; (i < myAirOutputs.length)
                && ((distributedAir.O2 + distributedAir.CO2
                        + distributedAir.other + distributedAir.nitrogen) > 0); i++) {
            ...
            airOutActualFlowRates[i] = breathAdded.O2 + breathAdded.CO2
                    + breathAdded.other + breathAdded.nitrogen;
        }

        //Get CO2 from stores/environment
        float gatheredCO2Air = getMostResourceFromStore(myCO2AirStoreInputs,
                CO2AirStoreInMaxFlowRates, CO2AirStoreInDesiredFlowRates,
                CO2AirStoreInActualFlowRates);
        for (int i = 0; i < myCO2AirEnvironmentInputs.length; i++) {
            float amountToTake = Math.min(CO2AirEnvironmentInMaxFlowRates[i],
                    CO2AirEnvironmentInDesiredFlowRates[i]);
            CO2AirEnvironmentInActualFlowRates[i] = myCO2AirEnvironmentInputs[i]
                    .takeCO2Moles(amountToTake);
            gatheredCO2Air += CO2AirEnvironmentInActualFlowRates[i];
        }
        //Push CO2 to stores/environment
        float CO2AirPushed = pushResourceToStore(myCO2AirStoreOutputs,
                CO2AirStoreOutMaxFlowRates, CO2AirStoreOutDesiredFlowRates,
                CO2AirStoreOutActualFlowRates, gatheredCO2Air);
        float CO2AirLeft = gatheredCO2Air - CO2AirPushed;
        for (int i = 0; (i < myCO2AirEnvironmentOutputs.length)
                && (CO2AirLeft > 0); i++) {
            float amountToPushFirst = Math.min(
                    CO2AirEnvironmentOutMaxFlowRates[i],
                    CO2AirEnvironmentOutDesiredFlowRates[i]);
            float amountToPushFinal = Math.min(amountToPushFirst, CO2AirLeft);
            CO2AirEnvironmentOutActualFlowRates[i] = myCO2AirEnvironmentOutputs[i]
        ...
        float gatheredO2Air = getMostResourceFromStore(myO2AirStoreInputs,
                O2AirStoreInMaxFlowRates, O2AirStoreInDesiredFlowRates,
                O2AirStoreInActualFlowRates);
        for (int i = 0; i < myO2AirEnvironmentInputs.length; i++) {
            float amountToTake = Math.min(O2AirEnvironmentInMaxFlowRates[i],
                    O2AirEnvironmentInDesiredFlowRates[i]);
            O2AirEnvironmentInActualFlowRates[i] = myO2AirEnvironmentInputs[i]
                    .takeO2Moles(amountToTake);
            gatheredO2Air += O2AirEnvironmentInActualFlowRates[i];
        }
        //Push O2 to stores/environment
        float O2AirPushed = pushResourceToStore(myO2AirStoreOutputs,
                O2AirStoreOutMaxFlowRates, O2AirStoreOutDesiredFlowRates,
                O2AirStoreOutActualFlowRates, gatheredO2Air);
        float O2AirLeft = gatheredO2Air - O2AirPushed;
        for (int i = 0; (i < myO2AirEnvironmentOutputs.length)
                && (O2AirLeft > 0); i++) {
            float amountToPushFirst = Math.min(
                    O2AirEnvironmentOutMaxFlowRates[i],
                    O2AirEnvironmentOutDesiredFlowRates[i]);
            float amountToPushFinal = Math.min(amountToPushFirst, O2AirLeft);
            O2AirEnvironmentOutActualFlowRates[i] = myO2AirEnvironmentOutputs[i]
        ...
        float gatheredNitrogenAir = getMostResourceFromStore(
                myNitrogenAirStoreInputs, nitrogenAirStoreInMaxFlowRates,
                nitrogenAirStoreInDesiredFlowRates,
                nitrogenAirStoreInActualFlowRates);
        for (int i = 0; i < myNitrogenAirEnvironmentInputs.length; i++) {
            float amountToTake = Math.min(
                    nitrogenAirEnvironmentInMaxFlowRates[i],
                    nitrogenAirEnvironmentInDesiredFlowRates[i]);
            nitrogenAirEnvironmentInActualFlowRates[i] = myNitrogenAirEnvironmentInputs[i]
        ...
        float nitrogenAirPushed = pushResourceToStore(
                myNitrogenAirStoreOutputs, nitrogenAirStoreOutMaxFlowRates,
                nitrogenAirStoreOutDesiredFlowRates,
                nitrogenAirStoreOutActualFlowRates, gatheredNitrogenAir);
        float nitrogenAirLeft = gatheredNitrogenAir - nitrogenAirPushed;
        for (int i = 0; (i < myNitrogenAirEnvironmentOutputs.length)
                && (nitrogenAirLeft > 0); i++) {
            float amountToPushFirst = Math.min(
                    nitrogenAirEnvironmentOutMaxFlowRates[i],
                    nitrogenAirEnvironmentOutDesiredFlowRates[i]);
            float amountToPushFinal = Math.min(amountToPushFirst,
                    nitrogenAirLeft);
            nitrogenAirEnvironmentOutActualFlowRates[i] = myNitrogenAirEnvironmentOutputs[i]
        ...
        float gatheredWaterAirMoles = getMostResourceFromStore(
                myWaterAirStoreInputs, waterAirStoreInMaxFlowRates,
                waterAirStoreInDesiredFlowRates, waterAirStoreInActualFlowRates);
        for (int i = 0; i < myWaterAirEnvironmentInputs.length; i++) {
            float amountToTake = Math.min(waterAirEnvironmentInMaxFlowRates[i],
        ...
        float waterAirPushedLiters = pushResourceToStore(
                myWaterAirStoreOutputs, waterAirStoreOutMaxFlowRates,
                waterAirStoreOutDesiredFlowRates,
                waterAirStoreOutActualFlowRates, gatheredWaterAirLiters);

        float waterAirLeft = gatheredWaterAirMoles
                - waterLitersToMoles(waterAirPushedLiters);
        for (int i = 0; (i < myWaterAirEnvironmentOutputs.length)
                && (waterAirLeft > 0); i++) {
            float amountToPushFirst = Math.min(
                    waterAirEnvironmentOutMaxFlowRates[i],
                    waterAirEnvironmentOutDesiredFlowRates[i]);
            float amountToPushFinal = Math.min(amountToPushFirst, waterAirLeft);
            waterAirEnvironmentOutActualFlowRates[i] = myWaterAirEnvironmentOutputs[i]
    ...
    public void setPotableWaterOutputs(PotableWaterStore[] destinations,
            float[] maxFlowRates, float[] desiredFlowRates) {
    ...
    public void setBiomassOutputs(BiomassStore[] destinations,
            float[] maxFlowRates, float[] desiredFlowRates) {
    ...
    public void setAirOutputs(SimEnvironment[] destinations,
            float[] maxFlowRates, float[] desiredFlowRates) {
    ...
    public void setCO2AirEnvironmentInputs(SimEnvironment[] sources,
            float[] maxFlowRates, float[] desiredFlowRates) {
    ...
    public void setCO2AirEnvironmentOutputDesiredFlowRate(float amount,
            int index) {
    ...
    public void setCO2AirStoreOutputs(CO2Store[] destinations,
            float[] maxFlowRates, float[] desiredFlowRates) {
    ...
    public void setCO2AirEnvironmentOutputs(SimEnvironment[] destinations,
            float[] maxFlowRates, float[] desiredFlowRates) {
    ...
    public void setO2AirEnvironmentInputs(SimEnvironment[] sources,
            float[] maxFlowRates, float[] desiredFlowRates) {
    ...
    public void setO2AirStoreOutputs(O2Store[] destinations,
            float[] maxFlowRates, float[] desiredFlowRates) {
    ...
    public void setO2AirEnvironmentOutputs(SimEnvironment[] destinations,
            float[] maxFlowRates, float[] desiredFlowRates) {
    ...
    public void setNitrogenInputs(NitrogenStore[] sources,
            float[] maxFlowRates, float[] desiredFlowRates) {
    ...
    public void setNitrogenOutputs(NitrogenStore[] destinations,
            float[] maxFlowRates, float[] desiredFlowRates) {
    ...
    public void setNitrogenAirEnvironmentInputMaxFlowRate(float amount,
            int index) {
    ...
    public void setNitrogenAirEnvironmentInputDesiredFlowRate(float amount,
            int index) {
    ...
    public void setNitrogenAirStoreInputs(NitrogenStore[] sources,
            float[] maxFlowRates, float[] desiredFlowRates) {
    ...
    public void setNitrogenAirEnvironmentInputs(SimEnvironment[] sources,
            float[] maxFlowRates, float[] desiredFlowRates) {
    ...
    public void setNitrogenAirEnvironmentOutputMaxFlowRate(float amount,
            int index) {
    ...
    public void setNitrogenAirEnvironmentOutputDesiredFlowRate(float amount,
            int index) {
    ...
    public void setNitrogenAirStoreOutputs(NitrogenStore[] destinations,
            float[] maxFlowRates, float[] desiredFlowRates) {
    ...
    public void setNitrogenAirEnvironmentOutputs(SimEnvironment[] destinations,
            float[] maxFlowRates, float[] desiredFlowRates) {
    ...
    public void setWaterAirEnvironmentInputDesiredFlowRate(float amount,
            int index) {
    ...
    public void setWaterAirStoreInputs(WaterStore[] sources,
            float[] maxFlowRates, float[] desiredFlowRates) {
    ...
    public void setWaterAirEnvironmentInputs(SimEnvironment[] sources,
            float[] maxFlowRates, float[] desiredFlowRates) {
    ...
    public void setWaterAirEnvironmentOutputDesiredFlowRate(float amount,
            int index) {
    ...
    public void setWaterAirStoreOutputs(WaterStore[] destinations,
            float[] maxFlowRates, float[] desiredFlowRates) {
    ...
    public void setWaterAirEnvironmentOutputs(SimEnvironment[] destinations,
            float[] maxFlowRates, float[] desiredFlowRates) {
