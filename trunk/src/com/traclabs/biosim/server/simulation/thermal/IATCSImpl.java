...
public class IATCSImpl extends SimBioModuleImpl implements
    ...
    //During any given tick, this much dry waste is needed for the IATCS
    // to run optimally
    private static final float DRY_WASTE_NEEDED_BASE = 10f;

    //During any given tick, this much O2 is needed for the IATCS to run
    // optimally
    ...
    //Flag to determine if the IATCS has enough dryWaste to function
    // nominally
    private boolean hasEnoughDryWaste = false;

    //Flag to determine if the IATCS has enough O2 to function nominally
    private boolean hasEnoughO2 = false;

    //The dryWaste consumed (in kilograms) by the IATCS at the current
    // tick
    ...
    //The oxygen consumed (in moles) by the IATCS at the current tick
    private float currentO2Consumed = 0f;

    //The CO2 produced (in moles) by the IATCS at the current tick
    ...
    /**
     * Returns the dryWaste consumed (in kilograms) by the IATCS during
     * the current tick
     * 
     * @return the dryWaste consumed (in kilograms) by the IATCS during
     *         the current tick
     */
    ...
    /**
     * Returns the O2 consumed (in moles) by the IATCS during the current
     * tick
     * 
     * @return the O2 consumed (in moles) by the IATCS during the current
     *         tick
     */
    ...
    /**
     * Returns the CO2 produced (in moles) by the IATCS during the current
     * tick
     * 
     * @return the CO2 produced (in moles) by the IATCS during the current
     *         tick
     */
    ...
    /**
     * Checks whether IATCS has enough dryWaste to run optimally or not
     * 
     * @return <code>true</code> if the IATCS has enough dryWaste,
     *         <code>false</code> if not.
     */
    ...
    /**
     * Checks whether IATCS has enough O2 to run optimally or not
     * 
     * @return <code>true</code> if the IATCS has enough O2,
     *         <code>false</code> if not.
     */
    ...
    /**
     * Attempts to collect enough O2 from the O2 Store to run the IATCS
