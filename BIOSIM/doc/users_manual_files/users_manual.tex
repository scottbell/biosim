...
preset critical levels the crew will abandon the mission and return
home.
...
Explain object oriented methodology and how that relates to BioSim.

\chapter{Installing the simulation}

\section{Installing an executable}

An executable that will run under most Windows operating systems.  You
can ownload the Windows executable from: {\tt www.traclabs.com/biosim}. 
...
from the number of crew members to the size of the biomass production
module.  The configuration of BioSim is controlled by an eXtensible
Markup Language (XML) file that is read in during BioSim
initialization.
...
\item DirtyWaterInFlowRateActuator:
\item DirtyWaterOutFlowRateActuator:
\item GreyWaterInFlowRateActuator:
\item GreyWaterOutFlowRateActuator:
\item PotableWaterInFlowRateActuator:
\item PotableWaterOutFlowRateActuator:
\end{enumerate}
...
\begin{enumerate}
\item PowerInFlowRateSensor:
\item PowerOutFlowRateSensor:
\item PowerStoreLevelSensor:
\end{enumerate}

\subsubsection{Actuators}

\begin{enumerate}
\item PowerInFlowRateActuator:
\item PowerOutFlowRateActuator:
\end{enumerate}
