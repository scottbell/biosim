			...
			try {
				if (myArgs[i].startsWith("-id=")) {
					try {
						...
						Logger.getLogger(GenericServer.class)
								.warn(
										"Problem parsing arguments on arg "
												+ myArgs[i]);
						e.printStackTrace();
					}
				...
				Logger
						...
						.error(
								"Bad command line arguments while trying to get ID. Should be of form: -id=N where N is an integer");
			}
		}
		return myID;
	}
		...
		for (int i = 0; i < myArgs.length; i++) {
			try {
				...
				Logger
						.getLogger(GenericServer.class)
						.error(
								"Bad command line arguments while trying to parse module name. Should be of form: -name=foo where foo is the name of the module");
			}
		}
		return myName;
	}
		...
		for (int i = 0; i < myArgs.length; i++) {
			try {
				...
				Logger
						.getLogger(GenericServer.class)
						.error(
								"Bad command line arguments while trying to parse configuration file. Should be of form: -xml=path/to/foo.xml where foo.xml is the name of the configuration file");
			}
		}
		return xmlLocation;
	}
