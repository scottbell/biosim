...
public class LognormalDecider extends FailureDecider {
	...
	protected double getReliability(double timeElapsed) {
		return lognormalReliability(myLogmean, myLogsd, timeElapsed);
	}
	 ...
	 * function and reliability function at x. 
	 *@param x time 
	 *@param logmean mu. 
	 *@param logsd sita
	 */
...
public static double lognormalReliability( double logmean, double logsd, double x) {
		double R;
		R=1-lognormalcdf(logmean,logsd, x);
		return R;
}	

public static double lognormalFailureRate( double logmean,double logsd, double x) {
		double z;
		double y;
		y = (java.lang.Math.log(x) - logmean) / logsd;
