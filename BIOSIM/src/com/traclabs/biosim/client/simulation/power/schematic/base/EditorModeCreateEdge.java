...
public class EditorModeCreateEdge extends ModeCreate {
	...
	private Class getUserClassChoice(Class[] classesMeetingContraints) {
		...
		String resourceStringSelected = (String) JOptionPane.showInputDialog(
				new JFrame(), "Select a flowrate to use", "Flowrate Selection",
				JOptionPane.QUESTION_MESSAGE, null,
				userFriendlyResourceStrings, userFriendlyResourceStrings[0]);

		int selectedIndex = getIndexOfString(userFriendlyResourceStrings,
				resourceStringSelected);
		Class classSelected = classesMeetingContraints[selectedIndex];
		return classSelected;
	}

	private int getIndexOfString(String[] userFriendlyResourceStrings,
			String resourceStringSelected) {
	...
	private String[] generateUserFriendlyResources(
			Class[] classesMeetingContraints) {
		...
		for (int i = 0; i < userFriendlyArray.length; i++) {
			...
			String withoutOperations = classString.substring(0, classString
					.indexOf("Operations"));
			...
			String nicelySpaced = withoutOperations.substring(0,
					indexToAddSpace)
					+ " " + withoutOperations.substring(indexToAddSpace);
