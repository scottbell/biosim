...
public class ModulePanel extends JPanel {
    ...
    public ModulePanel() {
        ...
        myAllModulesList.addListSelectionListener(new ModuleListSelectionListener());
        
        String[] mySimModuleNames = BioHolderInitializer.getBioHolder().theBioDriver.getSimModuleNames();
        ...
        mySimModulesList.addListSelectionListener(new ModuleListSelectionListener());
        
        String[] mySensorModuleNames = BioHolderInitializer.getBioHolder().theBioDriver.getSensorNames();
        Arrays.sort(mySensorModuleNames);
        mySensorModulesList = new JList(mySensorModuleNames);
        mySensorModulesList.setSelectionMode(ListSelectionModel.SINGLE_SELECTION);
        mySensorModulesList.setSelectedIndex(0);
        mySensorModulesList.addListSelectionListener(new ModuleListSelectionListener());
        
        String[] myActuatorModuleNames = BioHolderInitializer.getBioHolder().theBioDriver.getActuatorNames();
        Arrays.sort(myActuatorModuleNames);
        myActuatorModulesList = new JList(myActuatorModuleNames);
        myActuatorModulesList.setSelectionMode(ListSelectionModel.SINGLE_SELECTION);
        myActuatorModulesList.setSelectedIndex(0);
        myActuatorModulesList.addListSelectionListener(new ModuleListSelectionListener());
        ...
        myTabbedPane.addTab("Actuators", new JScrollPane(myActuatorModulesList));
    ...
    private void notifyListeners(){
        for (Iterator iter = myListeners.iterator(); iter.hasNext();){
            ((ModuleSelectionListener)(iter.next())).stateChanged(new ChangeEvent(this));
        }
    }
